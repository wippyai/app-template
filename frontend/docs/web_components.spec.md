...
2. **Web Page** - Provides a complete HTML page that can be loaded in an iframe or new window

Web Pages follow regular ./iframe-proxy.md API
...
```json
{
  ...
  "scripts": {
    ...
    "build:debug": "esbuild src/index.ts --bundle --outfile=dist/index.js --format=esm --sourcemap",
    "dev": "esbuild src/index.ts --bundle --outfile=dist/index.js --format=esm --watch",
    "test": "jest"
  }
}
```
...
- **`wippy.path`** - HTML entry point - **Required for Web page only**
- **`wippy.proxy`** - Proxy configuration - **Required for Web page only**
...
### `wippy.path` (Web page)
Path to the HTML entry point that will be loaded in an iframe or new window.

### `wippy.proxy` (Web page only)
...
**Note**: All injection options default to `true` when `enabled` is `true`. The default configuration above provides the optimal balance of host integration and app autonomy.
...
Inside the page you **MUST** use the [`iframe-proxy`](./iframe-proxy.md) API to talk to the host unless you explicitly disabled it. Note you wont have access to host config, including auth token, like this.
...
**Only these libraries should be marked as external**

| Package | Purpose | Version |
|---------|---------|---------|
| `@wippy-fe/proxy` | Wippy proxy API (`api`, `host`, `on`, `form`) | Host-provided |
...
> **⚠︎  Do not bundle these libraries** – always import them directly and rely on the host import-map:
>
> ```ts
...
<body>
<h1 class="text-lg font-semibold">Hello from web page example</h1>
