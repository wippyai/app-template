...
export function createAppRouter(host: HostApi, on: OnSubscription | null, startPath?: string): Router {
  const router = createRouter({
    // Do NOT pass startPath to createMemoryHistory — use router.push() below instead
  ...
  // Handle initial path from host — use router.push() after full router creation
  if (startPath) {
...
- **Never bundle** Vue, Pinia, Vue Router, Luxon, or @wippy-fe/proxy
...
export async function createMainApp() {
    ...
    // Add router
    const router = createAppRouter(instance, `/${startPath}`)
    app.use(router)
...
- **Start path handling** - **MUST** use `router.push(normalized)` after full router creation (do NOT pass startPath to `createMemoryHistory()`)
...
- **Use official types**: Import Wippy API types from `@wippy-fe/types-global-proxy` for proper type safety
...
Use the official types provided by the `@wippy-fe/types-global-proxy` package:
...
This section provides a **complete, tested working example** of a Wippy page application. Copy these files exactly to avoid build errors.
...
<head>
    ...
    <!-- Import map for host-provided dependencies -->
    <script type="importmap">
        {
            "imports": {
                ...
                "primevue/dialog": "https://esm.sh/primevue@4/dialog?external=vue",
                "primevue/checkbox": "https://esm.sh/primevue@4/checkbox?external=vue",
...
import { createAppRouter } from './router'
import './styles.css'
import './theme-config.css'  // Will be populated by host injection
import './tailwind.css'
...
export async function createMainApp() {
  ...
  // Create router with host API for host communication
...
html, body {
  ...
  margin: 0;
  padding: 0;
  @apply bg-surface-100 dark:bg-surface-950 text-surface-900 dark:text-surface-100;
}
