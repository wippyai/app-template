...
2. `host` - Host (parent window) communication methods
...
The `host` object provides methods to communicate with the parent Wippy application.
...
The `on` function subscribes to events from the parent window and WebSocket layer.

### @history

Emitted when the parent URL changes (SPA navigation).
...
Emitted when iframe visibility changes (tab switch, hide/show). The parent watches a visibility ref and sends `VisibilityWasUpdatedInParent` messages.
...
4. All iframe → parent commands are bridged automatically
